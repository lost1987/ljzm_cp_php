 ...
 * 符文合成
 */
class FwhcDailyService extends ServerDBChooser{
    function FwhcDailyService(){
        include BASEPATH.'/Common/event.php';
        $list=array();

        for($i=0;$i<=count($gameevent);$i++){
            if(isset($gameevent[$i])){
                if(preg_match('/符文合成/',$gameevent[$i])){
                    array_push($list,$i);
                }
            }
        }
    ...
    public function lists($page,$condition){
        ...
        if(!empty($server)){
            ...
            $list = $this->db->select("a.id1,a.type,a.str as action,a.param2,a.param1,a.param4,$time
                                        as time,b.id,b.account_name,b.name,b.levels,b.xingji")
            ...
            foreach($list as &$obj){
                ...
                $obj->fwname = fetch_object_by_key('id',$obj->param1,$items) -> name;
                if($obj->type==1){
                    $obj->typename = '消耗';
