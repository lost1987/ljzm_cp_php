 ...
 * 经验
 */
class ExpDailyService extends ServerDBChooser{

    private $expid='90000004';

    function ExpDailyService(){
    ...
    public function getCondition($condition){
        ...
        if(empty($sql))
        return " where a.param1 = $this->expid";
        return " where a.param1 = $this->expid and ".$sql;
    ...
    public function lists($page,$condition){
        ...
        if(!empty($server)){
            ...
            $sql = "select * from (select row_number() over (order by a.time desc) as rownumber,
                     a.id1,a.type,a.str as action,a.param2,a.param4,CONVERT(varchar(20),  a.time, 120) as time,b.id,b.account_name,b.name,b.levels,b.exp from  $this->table_record a left join   $this->table_user b on a.id1=b.id  $consql )
                    as t where t.rownumber > $page->start and t.rownumber <= $page->limit";
            $time = $this->db->datetime('a.time');
            $list = $this->db->select("a.id1,a.type,a.str as action,a.param2,a.param4,$time as time,b.id,b.account_name,b.name,b.levels,b.exp")
