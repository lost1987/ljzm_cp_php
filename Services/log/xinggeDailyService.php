 ...
 * 性格卡合成
 */
class XinggeDailyService extends ServerDBChooser{
    function XinggeDailyService(){
        include BASEPATH.'/Common/event.php';
        $list=array();
        $length=170;
        for($i=0;$i<=$length;$i++){
            if(isset($gameevent[$i])){
                if(preg_match('/性格卡/',$gameevent[$i])){
                    array_push($list,$i);
                }
            }
        }
    ...
    public function lists($page,$condition){
        ...
        if(!empty($server)){
            ...
            foreach($list as &$obj){
                ...
                $obj->xgname = fetch_object_by_key('id',$obj->param1,$items) -> name;
                if($obj->type==1){
                    $obj->typename = '损失';
                    $obj->zbqhchange = '-'.$obj->param2;
                }
                else {
                    $obj->typename = '收益';
                    $obj->zbqhchange = '+'.$obj->param2;
