 ...
 * 培养日志
 */
class PeiyangDailyService extends ServerDBChooser{
    function PeiyangDailyService(){
        include BASEPATH.'/Common/event.php';
        $list=array();
        $length=170;
        for($i=0;$i<= $length;$i++){
            if(isset($gameevent[$i])){
                if(preg_match('/培养/',$gameevent[$i])){
                    array_push($list,$i);
                }
            }
        }
    ...
    public function lists($page,$condition){
        ...
        if(!empty($server)){
            ...
            foreach($list as &$obj){
                ...
                $obj->pyname = fetch_object_by_key('id',$obj->param1,$items) -> name;

                $obj->detail = empty($this->gameevent[$obj->param4]) ? '未知' : $this->gameevent[$obj->param4];

                if($obj->type==1){
                    $obj->typename = '损失';
                    $obj->pyname  .= '-'.$obj->param2;
                }
