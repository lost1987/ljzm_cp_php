 ...
 * 帮派创建解散日志
 */
class BangpaiDailyService extends ServerDBChooser{
    function BangpaiDailyService(){
        include BASEPATH.'/Common/event.php';
        $list=array();
        $lenght=170;
        for($i=0;$i<=$lenght;$i++){
            if(isset($gameevent[$i])){
                if(preg_match('/帮派/',$gameevent[$i])){
                    array_push($list,$i);
                }
            }
        }
    ...
    public function lists($page,$condition){
        ...
        if(!empty($server)){
            ...
            $list = $this->db->select("a.id as tid,a.id1,a.type,a.str as action,a.param2,a.param1,a.param3,a.param4,$time
                                    as time,b.id,b.account_name,b.name,b.levels,b.partyname")
            ...
            foreach($list as &$obj){
                ...
                $obj->bpname = empty(fetch_object_by_key('id',$obj->param1,$items) -> name) ? '未知' : fetch_object_by_key('id',$obj->param1,$items) -> name;
                if($obj->type==1){
                    $obj->typename = '损失';
                    $obj->bangpaichange = $obj->bpname.' -'.$obj->param2;
                }
                else {
                    $obj->typename = '收益';
                    $obj->bangpaichange = $obj->bpname.' +'.$obj->param2;
