 ...
 * 许愿日志
 */
class XuyuanDailyService extends ServerDBChooser{

    function XuyuanDailyService(){
        include BASEPATH.'/Common/event.php';
        $list=array();
        $length=170;
        for($i=0;$i<=$length;$i++){
            if(isset($gameevent[$i])){
                if(preg_match('/许愿/',$gameevent[$i])){
                    array_push($list,$i);
                }
            }
        }
    ...
    public function lists($page,$condition){
        ...
        if(!empty($server)){
            ...
            foreach($list as &$obj){
                ...
                $obj->xuyuanname = empty(fetch_object_by_key('id',$obj->param1,$items) -> name) ? '未知' : fetch_object_by_key('id',$obj->param1,$items) -> name;
                if($obj->type==1){
                    $obj->typename = '损失';
                    $obj->xuyuanchange = $obj->xuyuanname.' -'.$obj->param2;
                }
                else {
                    $obj->typename = '收益';
                    $obj->xuyuanchange = $obj->xuyuanname.' +'.$obj->param2;
