 ...
 */
class HuobankaDailyService extends ServerDBChooser{
    function HuobankaDailyService (){
        include BASEPATH.'/Common/event.php';
        $list=array();
        $length=170;
        for($i=0;$i<=$length;$i++){
            if(isset($gameevent[$i])){
                if(preg_match('/伙伴卡/',$gameevent[$i])){
                    array_push($list,$i);
                }
            }
        }
    ...
    public function num_rows($condition){
        ...
        $sql = "select count(a.id) as num from $this->table_record a left join
$this->table_user b on a.id1=b.id    $consql";
    ...
    public function lists($page,$condition){
        ...
        if(!empty($server)){
            ...
            $list = $this->db->select("a.id as tid,a.id1,a.type,a.str as action,a.param2,a.param1,a.param3,a.param4,$time
            ...
            foreach($list as &$obj){
                ...
                $obj->hbkname = empty(fetch_object_by_key('id',$obj->param1,$items) -> name) ? '未知' : fetch_object_by_key('id',$obj->param1,$items) -> name;
                if($obj->type==1){
                    $obj->typename = '损失';
