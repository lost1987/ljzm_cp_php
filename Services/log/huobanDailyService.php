 ...
 * 伙伴日志
 */
class HuobanDailyService extends ServerDBChooser{
    function HuobanDailyService(){
        include BASEPATH.'/Common/event.php';
        $list=array();
        $length=170;
        for($i=0;$i<=$length;$i++){
            if(isset($gameevent[$i])){
                if(preg_match('/伙伴/',$gameevent[$i])){
                    array_push($list,$i);
                }
            }
        }
    ...
    public function lists($page,$condition){
        ...
        if(!empty($server)){
            ...
            foreach($list as &$obj){
                ...
                $obj->hbname = empty(fetch_object_by_key('id',$obj->param1,$items) -> name) ? '未知' : fetch_object_by_key('id',$obj->param1,$items) -> name;
                if($obj->type==1){
                    $obj->typename = '损失';
